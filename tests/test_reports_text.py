...
class TestTextReportBasicStructure:
    ...
    async def test_generate_text_returns_string(self, sample_report_mixed):
        """Verify generate_text returns str type."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_mixed)
        assert isinstance(result, str)

    async def test_generate_text_contains_title(self, sample_report_mixed):
    ...
    async def test_generate_text_contains_site_name(self, sample_report_mixed):
        """Verify report contains site_name."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_mixed)
        assert "Test Site" in result

    async def test_generate_text_contains_period(self, sample_report_mixed):
    ...
    async def test_generate_text_contains_summary(self, sample_report_mixed):
...
class TestTextReportSeverityOrdering:
    ...
    async def test_generate_text_severe_before_medium(self, sample_report_mixed):
    ...
    async def test_generate_text_medium_before_low(self, sample_report_mixed):
...
class TestTextReportTieredDetail:
    ...
    async def test_generate_text_severe_full_detail(self, sample_report_severe_only):
        """Verify severe finding includes title, description, remediation."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_severe_only)
    ...
    async def test_generate_text_medium_includes_remediation(self, sample_report_mixed):
    ...
    async def test_generate_text_low_one_liner(self, sample_report_low_only):
        """Verify low finding is formatted as one-liner with title and count."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_low_only)
    ...
    async def test_generate_text_low_no_description(self, sample_report_low_only):
    ...
    async def test_generate_text_low_no_remediation(self, sample_report_low_only):
...
class TestTextReportEdgeCases:
    ...
    async def test_generate_text_no_severe_section_when_empty(self, sample_report_low_only):
        """Verify no SEVERE FINDINGS section when counts.severe == 0."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_low_only)
        assert "SEVERE FINDINGS" not in result

    async def test_generate_text_no_medium_section_when_empty(self, sample_report_low_only):
        """Verify no MEDIUM FINDINGS section when counts.medium == 0."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_low_only)
        assert "MEDIUM FINDINGS" not in result

    async def test_generate_text_no_low_section_when_empty(self, sample_report_severe_only):
        """Verify no LOW FINDINGS section when counts.low == 0."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_severe_only)
        assert "LOW FINDINGS" not in result

    async def test_generate_text_recurring_badge(self, sample_report_mixed):
    ...
    async def test_generate_text_action_required_when_severe(self, sample_report_mixed):
        """Verify ACTION REQUIRED appears when severe findings exist."""
        rg = ReportGenerator()
        result = await rg.generate_text(sample_report_mixed)
        assert "ACTION REQUIRED" in result

    async def test_generate_text_no_html_escaping(self):
        ...
        rg = ReportGenerator()
        result = await rg.generate_text(report)
...
class TestTextReportEmptyReport:
    ...
    async def test_generate_text_empty_report(self):
        ...
        rg = ReportGenerator()
        result = await rg.generate_text(empty_report)
