...
<task type="auto">
  <name>Task 2: Add ws_manager parameter to LogCollector</name>
  <files>src/unifi_scanner/logs/collector.py</files>
  <action>
Update LogCollector constructor to accept WebSocket manager:

**Add to constructor:**
- ws_manager: Optional[WebSocketManager] = None parameter
- Store reference as self._ws_manager

**Update imports:**
- Add: from unifi_scanner.api import WebSocketManager
- Add: from unifi_scanner.logs.ws_collector import WSLogCollector, WSCollectionError

This task only adds the parameter and imports. The fallback chain logic is in Task 3.
  </action>
  <verify>
```bash
...
Imports for WebSocketManager, WSLogCollector, WSCollectionError are present.
  </done>
</task>

<task type="auto">
  <name>Task 3: Implement WS fallback in collect() method</name>
  <files>src/unifi_scanner/logs/collector.py</files>
  <action>
Update collect() method to add WebSocket as first collection source:
...
1. **Try WebSocket first** (if ws_manager provided and running):
   ...
   - Store ws_events list (they supplement API events, not replace)
   - Catch WSCollectionError and log warning, set ws_events = []

2. **Try REST API** (existing logic, unchanged):
   - APILogCollector.collect() as before
   - Store as api_events

3. **Merge WS + API events:**
   - Combine ws_events + api_events
   - Deduplicate by (timestamp, message) tuple using dict keyed on tuple
   - merged_events = list({(e.timestamp, e.message): e for e in ws_events + api_events}.values())

4. **Try SSH fallback** (existing logic):
   - If API failed/insufficient and SSH enabled
   - Merge and deduplicate same way

**Fallback behavior (WS-04 requirement):**
- WS unavailable (manager is None) -> skip WS step entirely
- WS connected but no events -> continue (empty ws_events is fine)
- WS throws WSCollectionError -> log warning, ws_events = [], continue to REST
- REST unavailable too -> try SSH
- All fail -> LogCollectionError

**Logging:**
- Log which sources contributed to final result
- Example: log.info("log_collection_complete", sources=["ws", "api"], ws_count=15, api_count=42)
  </action>
  <verify>
```bash
mypy src/unifi_scanner/logs/collector.py --ignore-missing-imports
ruff check src/unifi_scanner/logs/collector.py
pytest tests/ -v -k "collector" --tb=short
```
  </verify>
  <done>
...
Logging indicates which sources contributed.
  </done>
</task>
