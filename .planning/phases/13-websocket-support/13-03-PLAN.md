...
1. start(base_url: str, site: str, cookies: dict, device_type: DeviceType, verify_ssl: bool = False) -> None:
   ...
   - Sets on_event callback to add to buffer
...
2. _run_loop() -> None:
   ...
   - Sets it as the current loop
   - Runs client.start() until stopped
   - Catches exceptions and logs them

3. drain_events() -> list[BufferedEvent]:
   - Called from sync context (main thread)
   - Uses asyncio.run_coroutine_threadsafe to call buffer.drain()
...
```
  ...
  <done>
WebSocketManager class exists with start(), drain_events(), stop(), is_running() methods.
