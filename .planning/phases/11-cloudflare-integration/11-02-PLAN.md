...
must_haves:
  truths:
    ...
    - "is_configured()=False causes silent skip with no cleanup needed (IntegrationRegistry.get_configured filters out unconfigured integrations)"
...
# Verify unconfigured integration is NOT included (silent skip behavior)
class UnconfiguredSettings:
    cloudflare_api_token = None
    cloudflare_account_id = None
    initial_lookback_hours = 24

unconfigured = IntegrationRegistry.get_configured(UnconfiguredSettings())
print(f'Unconfigured integrations: {[i.name for i in unconfigured]}')
assert len(unconfigured) == 0, 'Unconfigured cloudflare should be silently skipped'

print('Registration and silent-skip behavior verified!')
"
```
  </verify>
  <done>
CloudflareIntegration registered with IntegrationRegistry. Import chain works correctly. Unconfigured integration silently skipped (no cleanup needed).
  </done>
</task>
...
3. is_configured()=False results in integration being excluded from get_configured() (silent skip, no cleanup)
4. validate_config() warns when account_id missing
5. Integration registered with IntegrationRegistry
6. `python -m pytest tests/test_integrations.py -x -q` passes
</verification>

<success_criteria>
- CloudflareIntegration class exists with name, is_configured(), validate_config(), fetch()
- is_configured()=False causes silent skip (IntegrationRegistry.get_configured excludes it, no cleanup needed)
